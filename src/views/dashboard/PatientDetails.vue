...
// const getPatientAlgoData = ref();

const getSingleDeviceData = ref([]);
// const getBloodOxygenGraphData = ref([]);
// const getBodyTempGraphData = ref([]);
const getPatientSteps = ref([]);
const getHeartRateGraphData = ref([]);
// const params = ref(route.params);

const loadingStatus = ref([]);
loadingStatus.value = store.getters.loadingStatus;

// const getSingleDeviceData = store.getters["doctors/getSingleDeviceData"];
const getMacAddress = store.getters["doctors/getMacAddress"];
const getPatientAlgoData = store.getters["patientData/getPatientAlgoData"];
console.log(store.getters["patientData/getPatientAlgoData"]);
const getBloodOxygenGraphData = store.getters["patientData/getBloodOxygenGraphData"];
const getBodyTempGraphData = store.getters["patientData/getBodyTempGraphData"];
// const getPatientSteps = store.getters["patientData/getPatientSteps"];
// const getHeartRateGraphData = store.getters["patientData/getHeartRateGraphData"];

const details = async (data) => {
  await store.dispatch("doctors/getSingleDevice", data);
  await store.dispatch("patientData/getPatientAlgoData", data);
  await store.dispatch("patientData/getPatientBodyTempData", data);
  await store.dispatch("patientData/getPatientBloodOxygenData", data);
  await store.dispatch("patientData/getPatientStepsData", data);
  await store.dispatch("patientData/getPatientHeartRateData", data);
  await store.dispatch("patientData/getPatientEcgData", data);
};
...
const getSingleDevice = () => {
  store.dispatch["doctors/getSingleDevice"];
};

// Lifecycle hooks using Composition API
// onMounted(async () => {
//   // await getSingleDeviceData();
//   // getSingleDeviceData.value = store.getters["doctors/getSingleDeviceData"];
//   store.dispatch("doctors/getSingleDevice", params.value.id);
...
watchEffect(() => {
  const data = getSingleDeviceData.value;
  liveMessage.value = data[0]?.isOnline ? "Online" : "Offline";
});

// Lifecycle Hooks
onMounted(async () => {
  await details(route?.params?.id);
  getSingleDeviceData.value = store.getters["doctors/getSingleDeviceData"];
  console.log(store.getters[("patientData/getPatientAlgoData", data)]);
  // getBloodOxygenGraphData.value = store.getters["patientData/getBloodOxygenGraphData"];
  // getBodyTempGraphData.value = store.getters["patientData/getBodyTempGraphData"];

  // await getSingleDevice(route?.params.value?.id);
  // getSingleDeviceData.value = store.getters["doctors/getSingleDeviceData"];
  createConnection();
});
...
const displayAlgoData = async () => {
  ...
  try {
    let algoData = await getPatientAlgoData(payload);
  ...
  <div>
    ...
    <div class="patient-details-wrapper">
      ...
      <div v-else class="patient-header">
        ...
        <v-row row wrap>
          <v-col xs12 sm12 md12 class="mt-5">
